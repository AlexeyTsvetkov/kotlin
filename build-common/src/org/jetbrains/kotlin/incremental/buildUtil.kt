...
fun mapLookupSymbolsToFiles(
        ...
        log: (String)->Unit,
        logFile: (File)->String = { it.canonicalPath },
    ...
    for (lookup in lookupSymbols) {
        ...
        log("${lookup.scope}#${lookup.name} caused recompilation of: ${affectedFiles.map(logFile)}")
...
fun <Target> mapClassesFqNamesToFiles(
    ...
    for (cache in caches) {
        for (dirtyClassFqName in classesFqNames) {
            ...
            log("Class $dirtyClassFqName caused recompilation of: ${logFile(srcFile)}")
