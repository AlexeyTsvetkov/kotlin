...
public class JsInliner extends JsVisitorWithContextImpl {
    ...
    private void inline(@NotNull JsInvocation call, @NotNull JsContext context) {
        ...
        JsStatement statement = statementContext.getCurrentStatement();
        JsStatement replacement = getStatementReplacementWithCallInlined(statement.deepCopy(), call, inlineableResult, getFunctionContext().getScope());

        statementContext.removeCurrentStatement();
        InsertionPoint<JsStatement> insertionPoint = statementContext.getInsertionPoint();
        insertionPoint.insertAllBefore(flattenStatement(replacement));
    }
