...
private fun createSyncOutputTask(project: Project, kotlinTask: AbstractCompile, javaTask: AbstractCompile, kotlinAfterJavaTask: AbstractCompile?, variantName: String) {
    // if kotlinAfterJavaTask is not null then kotlinTask compiles stubs, so don't sync them
    val compileKotlinTask = kotlinAfterJavaTask ?: kotlinTask
    val kotlinDir = compileKotlinTask.destinationDir
    ...
    project.getAllTasks(/* recursive = */false)
            .flatMap { it.value }
            .filter { it !== kotlinAfterJavaTask && javaTask in it.taskDependencies.getDependencies(it) }
            .forEach { it.dependsOn(syncTask) }

    syncTask.dependsOn(kotlinTask.name)
    syncTask.dependsOn(javaTask.name)
